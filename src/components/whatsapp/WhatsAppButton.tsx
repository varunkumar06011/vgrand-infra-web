...
interface WhatsAppButtonProps {
  ...
  title?: string;
  showText?: boolean;
  /** Optional override for the lead `project` field sent to /api/leads */
  leadProject?: string;
  /** Optional override for the lead `message` field sent to /api/leads */
  leadNote?: string;
}
    ...
    try {
      await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          phone: phoneNumber,
          project: leadProject || projectName || 'General Interest',
          message: leadNote || `Interest initiated from ${variant} button`,
          source: 'WhatsApp Web'
        }),
      });
    } catch (err) {
